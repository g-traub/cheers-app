...
export default function BarPin(props) {
  const { barData, showBarInfos, isSelected } = props;

  return (
    <Marker key={`marker-${barData.properties.id}`} latitude={barData.geometry.coordinates[1]} longitude={barData.geometry.coordinates[0]} className="barPinMarker">
      <svg
        height={isSelected ? 50 : 35}
        viewBox="0 0 25 38"
        style={{
          cursor: 'pointer',
          fill: isSelected ? '#FFA000' : '#FFE082',
          stroke: '#FAFAFA',
          transform: `translate(${-(isSelected ? 35/ 2 : 20/ 2)}px,${-(isSelected ? 50 : 35)}px)`,
          pointerEvents: 'all'
        }}
        onClick={() => showBarInfos(barData)}
      >
        <path d={ICON} />
      </svg>
    </Marker>
